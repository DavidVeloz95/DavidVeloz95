<div align="center">

# Welcome to my portfolio! I'm David Veloz 👋
<!--
Biomedical & Mechatronics Engineer focused on Medical Technology, Imaging Systems, Computer Vision and Embedded Systems.
-->

<img src="https://i.imgur.com/9qe0Lg9.png" width="100%">

<br>

[![LinkedIn](https://custom-icon-badges.demolab.com/badge/LinkedIn-0A66C2?logo=linkedin-white&logoColor=fff)](https://www.linkedin.com/in/davidveloz)
[![GitHub followers](https://img.shields.io/github/followers/davidveloz?style=social)](https://github.com/davidveloz)

</div>

---

# About me

M. Sc. Biomedical Engineer with a multidisciplinary background in medical devices, electronics, embedded systems, computer vision and software development.

I enjoy building real-time systems that combine sensing, signal processing and intelligent software for healthcare and robotics applications. My experience includes hardware/software integration, medical technology development, sensor systems and scientific software engineering.

Currently, I am especially interested in:

- Medical Imaging
- Computer Vision
- Image-Guided Systems
- Embedded Systems
- Real-Time Signal Processing
- Medical Device Development
- Robotics & Sensor Integration
- Healthcare Technology

---

# Technical Skills

## Programming Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-orange?style=for-the-badge)

---

## Medical Imaging & Computer Vision

![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)
![PCL](https://img.shields.io/badge/Point%20Cloud%20Library-00599C?style=for-the-badge)
![DICOM](https://img.shields.io/badge/DICOM-0A66C2?style=for-the-badge)

- Medical image processing
- Motion analysis
- Point cloud processing
- Real-time movement tracking
- Sensor data visualization
- Image-based analysis systems

---

## Embedded Systems & Electronics

![Embedded Systems](https://img.shields.io/badge/Embedded%20Systems-00599C?style=for-the-badge)
![MSP430](https://img.shields.io/badge/TI%20MSP430-CC0000?style=for-the-badge)
![BLE](https://img.shields.io/badge/Bluetooth%20Low%20Energy-0082FC?style=for-the-badge)
![PCB Design](https://img.shields.io/badge/PCB%20Design-222222?style=for-the-badge)

- Mixed-signal PCB design
- Sensor integration
- BLE communication
- Test systems development
- Hardware validation
- Embedded system integration

---

# Featured Projects

## Medical Imaging Workstation *(currently in development)*

Desktop application for visualization and analysis of medical imaging data.

### Planned features
- DICOM loading and visualization
- CT/MRI slice navigation
- Windowing and contrast adjustment
- ROI tools and measurements
- 3D visualization
- Modular imaging architecture

**Tech stack:** Python • PyQt • pydicom • SimpleITK • VTK

---

## Wearable Motion Analysis System

Development of a wearable system for real-time kinematic data acquisition using IMU sensors.

### Features
- BLE communication
- Android application for sensor visualization
- Desktop software for data analysis
- Real-time sensor acquisition
- Data export and visualization

**Tech stack:** Kotlin • Python • BLE • IMU Sensors

---

## Kinect-Based Human Motion Analysis

Real-time human movement recognition system using Microsoft Kinect SDK.

### Features
- Human posture classification
- Skeleton tracking
- Real-time motion analysis
- Activity recognition algorithms

**Tech stack:** C++ • Kinect SDK • Computer Vision

---

## 3D Point Cloud Analysis for Robotics

Algorithms for traversability analysis and obstacle detection using 3D point clouds.

### Features
- Ground segmentation
- Surface normal analysis
- Obstacle detection
- Traversability estimation
- Robotics-oriented environment analysis

**Tech stack:** Python • Point Cloud Processing • Robotics

---

# Professional Interests

I am passionate about developing technologies at the intersection of:

- Medical Imaging
- Computer-Assisted Healthcare
- Medical Devices
- Robotics
- Intelligent Sensing Systems
- Real-Time Visualization
- Human Motion Analysis
- Neurotechnology

---

# Currently Learning

- Medical imaging software architecture
- DICOM workflows
- PyQt advanced GUI development
- VTK visualization
- Image-guided systems
- Advanced computer vision techniques

---

# Connect with me

[![LinkedIn](https://custom-icon-badges.demolab.com/badge/LinkedIn-0A66C2?logo=linkedin-white&logoColor=fff)](https://www.linkedin.com/in/davidveloz)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?logo=gmail&logoColor=white)](mailto:david.veloz1202@gmail.com)

📫 Feel free to connect or collaborate on MedTech, imaging or embedded systems projects.

<!--
**DavidVeloz95/DavidVeloz95** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
